...
let allFiles = []; // 存放全部资源文件
let currentType = null; // 当前分类范围限制搜索
...
function showResources(type, subtype) {
  ...
  if (!matched.length) {
    document.getElementById("subcategoryTitle").textContent = "未找到资源";
    return;
  }
...
document.getElementById("searchInput").addEventListener("input", function (e) {
  ...
  allFiles.forEach(file => {
    if (currentType && file.type !== currentType) return; // 限定当前分类
